...
var initStats = function() {
    ...
    document.getElementById("Stats-output0").appendChild(stats.domElement);
    return stats;
}
document.onkeydown=function(event) {
    var e = event || window.event;
    if(e && e.keyCode==113) { 
    	let stats_div = document.getElementById("Stats-output0");
...
$(function() {
    ...
    socket.on('chat1 message', function(msg) {
    	...
    	var imgWith = $("#td" + n).width();
		var imgHeight = $("#td" + n).height();
	...
	if(openSocketIo) {
		socket.on('h264', function(msg) {
		    	...
		    	if(players[n] === undefined) {
		        	var canvas0 = document.getElementById('canvas' + n);
		        	canvas0.style.display='block';
		        	canvas0.style.zIndex=1;
		        	canvas0.style.position= 'static';
		        	canvas0.style.top=0;
		        	canvas0.style.left=0;
		        	canvas0.style.bottom=0;
		        	canvas0.style.right=0;
		        	if(canvas0 == null) {
		        		canvas0 = document.createElement('canvas' + n);
		    		 	canvas0.id = 'video-canvas' + n;
		    		 	canvas0.className = 'video-canvas';
		        	}
	...
	socket.on('del rtsp', function (data) {
		...
		setTimeout(function() {
			var canvas0 = document.getElementById('video-canvas' + data);
	    	if(canvas0 != null) {
	    		canvas0.parentNode.removeChild(canvas0);
	    	}
		}, 1000);
	});
